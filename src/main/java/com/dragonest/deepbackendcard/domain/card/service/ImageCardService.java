...
import com.dragonest.deepbackendcard.domain.card.exception.ImageNotFoundException;
...
public class ImageCardService {
    ...
    public void create(ImageCardRequest request, MultipartFile imageFile, String token) {
        ...
        ImageUploadResponse imageUploadResponse = webClient.imageUpload(String.format("%s%s", imageServerPath, "/v1/api/images/image"), imageServerKey, builder);
        if (imageUploadResponse.getCode() == 201) {
            imageCardRepository.save(request.toEntity(user.getUserId(), imageUploadResponse.getData().getIdent()));
        }
    }
    ...
    public void update(ImageCardRequest request, String id, String token) {
        ...
        imageCard.update(request.getName(), request.getCompany(), request.getAddress(), request.getPosition(), request.getMobile(), request.getTel(), request.getEmail(), request.getFax(), request.getHomepage(), request.getDepartment());
    ...
    public void delete(String id, String token) {
        ValidateData user = validate(token);
        ImageCard imageCard = imageCardRepository.findById(Long.valueOf(id)).orElseThrow(() -> ImageCardNotFoundException.EXCEPTION);
        webClient.removeImage(String.format("%s%s%s", imageServerPath, "/v1/api/images/", imageCard.getImage()), imageServerKey);
